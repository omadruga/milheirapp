// Corrige 2 erros históricos no banco Latam:
//   1) Deleta #286 (FLIGHT duplicada de 10.995 em 30/11/2024)
//   2) Atualiza #289 miles 10.696 → 10.969 (typo no cadastro)
// Depois recalcula saldo Latam.
...
async function calculate(accountId) {
  ...
  console.log(`  recalculado Latam: miles=${miles} avgPrice=${averagePrice.toFixed(4)} seats=${seatsUsed}/${seats}`);
}
...
async function main() {
  ...
  before.forEach((t) => console.log(`  #${t.id} ${t.date.toISOString().slice(0,10)} ${t.type} miles=${t.miles}`));

  console.log("\n=== Aplicando correções ===");
  const dup = await prisma.transaction.delete({ where: { id: 286 } });
  console.log(`  DELETE #${dup.id}`);
  const fix = await prisma.transaction.update({ where: { id: 289 }, data: { miles: 10969 } });
  console.log(`  UPDATE #${fix.id} miles → ${fix.miles}`);
  ...
  console.log(`  miles=${latam.miles?.toLocaleString()} avgPrice=${latam.averageMilePrice?.toFixed(4)} seats=${latam.seatsUsed}/${latam.seats}`);
}
